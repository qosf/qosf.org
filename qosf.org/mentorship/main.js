...
function getProviderLogoSrc(provider) {
    ...
    if (!p || p.includes('local'))
        return null;
    if (p.includes('ibm'))
        return 'public/ibm.png';
    if (p.includes('aws') || p.includes('amazon') || p.includes('braket'))
        return 'public/aws.png';
    if (p.includes('origin'))
        return 'public/origin.png';
    if (p.includes('quantinuum'))
        return 'public/quantinuum.png';
    return null;
}
...
function renderProviderLogoHtml(provider, accent = false) {
    const src = getProviderLogoSrc(provider);
    if (!src && String(provider || '').toLowerCase().includes('local'))
        return '';
...
function renderPlatformsTable() {
    ...
    tbody.querySelectorAll('td.compare-col').forEach((cell) => {
        cell.textContent = '';
    });
